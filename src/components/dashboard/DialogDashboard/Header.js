...
export default function Header({ dialogDashboard, dialog, setDialog }) {
    const options = ["Enable", "Disable"];

    if (dialogDashboard?.editing)
        return (
            <div className="flex align-items-center justify-content-between">
                <span className="font-bold white-space-nowrap">Editing</span>
                <SelectButton
                    pt={{
                        button: { className: `px-3 py-2 ${TEXT_NORMAL}` },
                    }}
    ...
    return <span className="font-bold white-space-nowrap">{dialogDashboard?.title}</span>;
}
